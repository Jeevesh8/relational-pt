...
def get_transition_mat(pt_transition_mat: Optional[jnp.ndarray]=None) -> jnp.ndarray:
    ...
    random_transition_mat = rng.uniform(size=(5,5), dtype=np.float32,)+0.05
    ac_dict = data_config["arg_components"]
    
    random_transition_mat[[ac_dict["I-C"], ac_dict["other"]], ac_dict["I-P"]] = -np.inf
    random_transition_mat[[ac_dict["I-P"], ac_dict["other"]], ac_dict["I-C"]] = -np.inf
    random_transition_mat[ac_dict["other"], [ac_dict["I-P"], ac_dict["I-C"]]] = -np.inf
    
    random_transition_mat[ac_dict["B-C"], ac_dict["I-P"]] = -np.inf
    random_transition_mat[ac_dict["B-P"], ac_dict["I-C"]] = -np.inf

    random_transition_mat[[ac_dict["B-C"], ac_dict["B-P"]], [ac_dict["B-C"], ac_dict["B-P"]]] = pt_transition_mat[0,0]

    random_transition_mat[ac_dict["B-C"], ac_dict["I-C"]] = pt_transition_mat[0,1]
    random_transition_mat[ac_dict["B-P"], ac_dict["I-P"]] = pt_transition_mat[0,1]

    random_transition_mat[[ac_dict["I-P"], ac_dict["I-C"]], [ac_dict["B-P"], ac_dict["B-C"]]] = pt_transition_mat[1,0]
    
    random_transition_mat[ac_dict["I-C"], ac_dict["I_C"]] = pt_transition_mat[1,1]
    random_transition_mat[ac_dict["I-P"], ac_dict["I-P"]] = pt_transition_mat[1,1]
    
    return jnp.array(random_transition_mat)

def get_params_dict(key: jnp.ndarray, base_model, pt_wts_file: str = None, use_pt_for_heads: bool=True) -> Dict:
    """
    Args:
    ...
    if use_pt_for_heads:
        ft_params["comp_predictor"]["linear"] = jax.tree_util.tree_multimap(copy_weights, (pt_params["comp_predictor"]["linear"], ft_params["comp_predictor"]["linear"]))
        ft_params["comp_predictor"]["transition_matrix"] = get_transition_mat(pt_params["comp_predictor"]["transition_matrix"])

        ft_params["relation_predictor"] = jax.tree_util.tree_multimap(copy_weights, (pt_params["relation_predictor"], ft_params["relation_predictor"]))
    
    else:
        ft_params["comp_predictor"]["transition_matrix"] = get_transition_mat()
        
    ft_params["comp_predictor"] = to_immutable_dict(ft_params["cmop_predictor"])
