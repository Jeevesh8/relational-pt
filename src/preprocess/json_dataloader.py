...
class load_reddit_data:
    def __init__(self, config, mode="train"):
        ...
        self.data_file = ("train_period_data.jsonlist" if mode == "train" else
                          "heldout_period_data.jsonlist")
        if self.mask_dms:
            self.dms = self.get_discourse_markers(
                config["discourse_markers_file"])
    ...
    def correct_replies(post_tree: dict) -> dict:
        ...
        for comment in post_tree["comments"].values():
            ...
            if (parent_id != post_tree["id"] and comment["id"]
                    not in post_tree["comments"][parent_id]["replies"]):
                post_tree["comments"][parent_id]["replies"].append(
                    comment["id"])
