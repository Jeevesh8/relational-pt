...
if __name__=='__main__':
    ...
    parser.add_argument('--data_folders', type=str, nargs='+', help='Folders having data JSON files extracted from reddit.')
    parser.add_argument('--write_folder', type=str, help='The folder where to write the processed data.')
    parser.add_argument('--valid_size', type=int, default=0, help="Percentage of data outside train, to keep in validation data. For 10%, specify 10. Default is 0.")
    ...
    write_processed_trees(os.path.join(args.write_folder, 'train.txt'), subtrees)
    del subtrees
    ...
    subtrees = process_trees(all_post_trees)
    write_processed_trees(os.path.join(args.write_folder, 'valid.txt'), subtrees[:(args.valid_size*len(subtrees))//100])
    write_processed_trees(os.path.join(args.write_folder, 'test.txt'), subtrees[(args.valid_size*len(subtrees))//100:])
    del subtrees
