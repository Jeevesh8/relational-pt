...
def dict_to_full_tree(tree: dict) -> Tuple[dict, str]:
    ...
    tree['comments'][tree['id']]['body'] = "<title> "+tree['title']+" </title> "+tree['selftext']
    tree['comments'][tree['id']]['replies'] = [id for id, comment in tree['comments'].items() if comment['parent_id']==tree['id']]
    return tree['comments'], tree['id']

def size_subtrees(tree: dict, root: str, tokenizer=get_tokenizer(), extra_tokens: int=1):
    """
    Args:
    ...
    def core_recursion(tree: dict, root: str):
        ...
        for reply_id in tree[root]['replies']:
            self_length += core_recursion(tree, reply_id)-2                    #-2 for <s> </s> tokens, they are already counted in initial val of self_length
...
def make_text_from_trees(tree: dict, root: str, preorder: bool=True, shuffle: bool=False)-> str:
    """
    ...
    def core_recursion(tree: dict, root: str, authors: dict) -> str:
        ...
        author, post_id, parent_id = tree[root]['author'], tree[root]['id'], tree[root]['parent_id']
        user_no = authors[author]
        
        node_text = "<post"+post_nos[post_id]+" parent_id= "+(str(None) if parent_id=='' else post_nos[parent_id])+"> "+ \
                        "<user"+user_no+"> "+ \
        ...
        for i, reply_id in enumerate(tree[root]['replies']):
            subtree_text += (" " if i!=0 else "") + core_recursion(tree, reply_id, authors)
...
def subtrees_lis(tree: dict, root_node: str, threshold: int=4096+5, preorder:bool=True, shuffle:bool=True)-> List[str]:
    """
    Args:
    ...
    def core_recursion(tree: dict, root: str) -> int:
        ...
        if tree[root]['subtree_size']<threshold or tree[root]['replies']==[]:
            ...
            tree = propagate_difference(tree, root_node, root, tree[root]['subtree_size'])
        else:
