...
class tree_crf(hk.Module):
    ...
    def _mst(self, log_energies: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        mst_energy = jnp.array([0.])
        edges = []

        for _ in range(M):
            max_index = jnp.where(log_energies==jnp.max(log_energies, keepdims=True))
            max_energy = log_energies[max_index]
            updatable_sample = jnp.logical_not(jnp.isneginf(max_energy))
            mst_energy += jnp.where(updatable_sample, max_energy, 0.)
            max_link = jnp.where( jnp.stack([updatable_sample]*n_rel_types, axis=-1), 
                                 jnp.squeeze(jnp.transpose(jnp.array(max_index))), 0.)
            ...
            log_energies = jax.ops.index_update(log_energies, (from_partition, jnp.expand_dims(to_partition, axis=-1), jnp.arange(n_rel_types)), -jnp.inf)
            log_energies = jax.ops.index_update(log_energies, (to_partition, jnp.expand_dims(from_partition, axis=-1), jnp.arange(n_rel_types)), -jnp.inf)
            ...
            partitions = jax.ops.index_update(partitions, (jnp.expand_dims(temp_idx, axis=-1), jnp.arange(M)), temp)
    ...
    def _score_tree(self, log_energies: jnp.ndarray, tree: jnp.ndarray) -> jnp.ndarray:
        ...
        """
        new_log_energies = jax.ops.index_update(log_energies, (0,0,0), 0.)
        score = jnp.array([0.])
        for edge in tree:
    ...
    def disc_loss(self, log_energies: jnp.ndaray, label_tree: jnp.ndarray) -> jnp.ndarray:
