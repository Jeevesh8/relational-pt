...
class relational_model(hk.Module):
    ...
    def _call(self, embds: jnp.ndarray,
        ...
        indices = (jnp.where(
            choices[:(self.max_comps - 1)] == num_all_embds,
            -1,
            choices[:(self.max_comps - 1)],
        ) + 1)
        lower_indices = jax.ops.index_update(
            jnp.roll(indices, shift=(1, ), axis=(0, )), (0, ), 0)

        from_embds, _ = jax.lax.scan(
            lambda carry, i: (
                jnp.where(
                    jnp.expand_dims(jnp.logical_and(lower_indices <= i, i <
                                                    indices),
                                    axis=-1),
                    embds[i] + carry,
                    carry,
            ...
            init=jnp.zeros((indices.shape[0], embds.shape[-1]),
                           dtype=embds.dtype),
            xs=jnp.arange(embds.shape[0]),
        )

        from_embds = from_embds / jnp.expand_dims(jnp.where(
            indices - lower_indices, indices - lower_indices, 1),
                                                  axis=-1)
        ...
        pad_mask = jnp.pad(indices != 0, pad_width=(1, 0), constant_values=1)
