...
def get_hf_model(tokenizer_len: int, token_types:int = 2, key = jax.random.PRNGKey(65)):
    ...
    """
    model = FlaxAutoModel.from_pretrained(stable_config["checkpoint"], vocab_size=tokenizer_len)
    ...
    if tokenizer_len>n_words:
        ...
        additional_embeds = jax.random.normal(subkey, shape=(tokenizer_len-n_words, embed_dim), dtype=original_embeds.dtype)
        model.params["embeddings"]["word_embeddings"]["embedding"] = jnp.concatenate([original_embeds, additional_embeds])
    
    original_embeds = model.params["embeddings"]["token_type_embeddings"]["embedding"]
    n_token_types, embed_dim = original_embeds.shape
    if token_types>n_token_types:
        key, subkey = jax.random.split(key)
        additional_embeds = jax.random.normal(subkey, shape=(token_types-n_token_types, embed_dim), dtype=original_embeds.dtype)
        model.params["embeddings"]["token_type_embeddings"]["embedding"] = jnp.concatenate([original_embeds, additional_embeds])
