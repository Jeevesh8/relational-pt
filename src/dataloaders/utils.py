...
def dict_to_inputs(tree: List[dict], tokenizer=get_tokenizer(), get_mat: bool=False, one_indexed: bool=True) -> Tuple[List[int], List[int], List[int], List[List[int]]]:
    """
    ...
    if get_mat:
        relations = [[0 for __ in len(tree)+idxing] for _ in len(tree)+idxing]
    else:
        relations = []

    for post, post_encoding in zip(tree, encodings):
        post_tags += [config['post_tags']['B']]+[config['post_tags']['I']]*(len(post_encoding)-1)
        #print(post['user_no'], len(config['user_tags']))
        user_tags += [config['user_tags'][post['user_no']]['B']]+[config['user_tags'][post['user_no']]['I']]*(len(post_encoding)-1)
        
        if post['parent_id'] is not None:
            if get_mat:
                relations[post['post_id']+idxing][post['parent_id']+idxing] = 1
            else:
                relations.append((post['post_id']+idxing, post['parent_id']+idxing))
...
modelInp = namedtuple('modelInp', ['input_ids', 'post_tags', 'user_tags', 'relations'])
