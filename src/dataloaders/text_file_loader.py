...
def get_all_trees(read_file):
    with open(read_file) as f:
        post_trees = [elem.strip() for elem in f.readlines() if not elem.startswith('-'*14)]

    comment_pattern = r'<post(\d+) parent_id= (.*?)> <user(\d+)>(.+?)<\/user\d+> <\/post\d+>'
    tree_with_post_parent_user_no = [re.findall(comment_pattern, post_tree) for post_tree in post_trees]
...
def format_data(tree_with_post_parent_user_no):
    with Pool(5) as p:
        tree_with_post_parent_user_no = p.map(only_inside_links, tree_with_post_parent_user_no)
        tree_with_post_parent_user_no = p.map(tree_ids_to_nos, tree_with_post_parent_user_no)
...
def get_tfds_dataset(file_lis, config):
    dataset = tf.data.Dataset.from_generator(get_generator(file_lis),
                                             output_signature=(tf.TensorSpec(shape=(None), dtype=tf.int32, name='input_ids'),
    ...
    dataset = dataset.padded_batch(config['batch_size'],
                                   padded_shapes=([stable_config['max_len']], [stable_config['max_len']], [stable_config['max_len']], [stable_config['max_comps'], 3]),
                                   padding_values=(config['pad_for']['input_ids'],
