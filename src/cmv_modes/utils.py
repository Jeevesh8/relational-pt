...
def is_internal(enclosing_string: str, begin: int, end: int, prev_str: Optional[str]=None)->bool:
    ...
    return ((begin > 0 and enclosing_string[begin - 1].isalnum()) or
            (end < len(enclosing_string) and enclosing_string[end].isalnum()) or
            (begin == 0 and (True if prev_str is None else prev_str[-1].isalnum()) and enclosing_string[0].isalnum()))

def has_space(enclosing_string: str, idx: int, preceeding:bool=True, prev_str: Optional[str]=None)->bool:
    ...
    if preceeding:
        return (    (idx==0
                     and (True if prev_str is None else prev_str[-1]==" ")
                    ) 
...
def modified_mask_encodings(
    ...
    while True:
        ...
        if not is_internal(iter_txt, idx, idx+len(marker),prev_str=prev_part):
