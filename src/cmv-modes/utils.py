...
def modified_mask_encodings(encoding: List[int], tok, markers: List[str]) -> Tuple[List[int], List[List[int]]]:
...
#    print("\n\n")
    ...
    seqs_to_mask = []
    while True:
        found_markers_start_pos = [iter_txt.lower().find(marker.lower()) for marker in markers]
        idx = min(filter(lambda x: x>=0, found_markers_start_pos), default=-1)
...
#        print("Marker: ", marker, "index: ",  idx, " in ", iter_txt[max(0,idx-10):idx+10])
        if not ((idx>0 and iter_txt[idx-1].isalnum()) or (idx+len(marker)<len(iter_txt) and iter_txt[idx+len(marker)].isalnum()) or (idx==0 and len(new_txt_parts)>0 and new_txt_parts[-1][-1].isalnum() and iter_txt[0].isalnum())):                    #if the match is not internal to a word

            new_txt_parts.append(iter_txt[:idx])

            if not (idx==0 or iter_txt[idx-1]==' '):                                                                      #if there isn't any pre-existing space, preceeding the marker
                new_txt_parts.append(' ')                                                                                 #add a space before the markeri

            new_txt_parts.append(iter_txt[idx:idx+len(marker)])

            if not (idx+len(marker)==len(iter_txt) or iter_txt[idx+len(marker)]==' '):                                    #if there isn't any pre-existing space, following the marker
                new_txt_parts.append(' ')                                                                                 #add a space after the marker

            words_before = iter_txt[:idx].strip().split()
            if idx==0 or (len(words_before)>0 and words_before[-1] in special_tokens):                                    #Since, tokenization is same for "[STARTQ] On the.." and "[STARTQ]On the.." spaces following special tokens are removed while tokenization.
...
def reencode_mask_tokens(encoding: List[int], tok, markers: List[str]) -> Tuple[List[int], List[int]]:
    """
    Args:
...
#   print("Original encoding: ", tok.decode(encoding))
    new_encoding, seqs_to_mask = modified_mask_encodings(encoding, tok, markers)
