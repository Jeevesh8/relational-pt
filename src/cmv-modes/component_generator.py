...
def get_components(component: bs4.BeautifulSoup, parent_type: Optional[str]=None, parent_id: Optional[str]=None, parent_refers:Optional[str]=None, parent_rel_type:Optional[str]=None):
    ...
    def chain_yield(comp_type='claim'):
        ...
        for part in parsed_component.find(comp_type).contents:
            ...
            if not str(part).strip().startswith('<claim') and not str(part).strip().startswith('<premise') and not part==parsed_component.find(comp_type).contents[0]:
                parent_ref = parent_id
                parent_id += 'Ć'
                parent_rel_type = 'cont'

            for _ in get_components(part, comp_type, parent_id, parent_refers, parent_rel_type):
                yield _
...
def generate_components(filename):
    ...
    if len(re.findall(r'\&\#.*;', str(parsed_xml)))!=0:
        raise AssertionError("HTML characters still remaining in XML: "+str(re.findall(r'\&\#.*;', str(parsed_xml))))

    user_dict = dict()

    yield (parsed_xml.find('title').get_text(),
