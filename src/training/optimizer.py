...
def get_adam_opt():
    ...
    lr_schedule = make_lr_schedule(warmup_percentage=0.1, total_steps=total_steps, restart_from=config["opt"]['restart_from'])

    opt = optax.chain(
                optax.identity() if config["opt"]["max_grad_norm"] is None else optax.clip_by_global_norm(config["opt"]['max_grad_norm']),
                optax.adam(learning_rate=config["opt"]["lr"]) if config["opt"]["weight_decay"] is None else optax.adamw(learning_rate=config["opt"]["lr"], weight_decay=config["opt"]["weight_decay"], mask=decay_mask_fn),
                optax.scale_by_schedule(lr_schedule) if config["opt"]["use_schedule"] else optax.identity(),
            )
