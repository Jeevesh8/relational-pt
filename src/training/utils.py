...
def relational_metric(prediction: List[List[int,int,int]], reference: List[List[int, int, int]]) -> Tuple[int,int]:
    """
    Args:
    ...
    return len(set_prediction.intersection(set_reference)), len(set_prediction.union(set_reference))
...
def batch_to_relational_lists(predictions: jnp.ndarray, references: jnp.ndarray):
    ...
    """
    batchwise_num_pred_rels = jnp.sum(jnp.sum(predictions==config['pad_for']['relations'], axis=-1)!=3, axis=-1)
    batchwise_num_ref_rels = jnp.sum(jnp.sum(references==config['pad_for']['relations'], axis=-1)!=3, axis=-1)
    
    return predictions.tolist(), references.tolist(), batchwise_num_pred_rels.tolist(), batchwise_num_ref_rels.tolist()
...
class relation_match_metric:
    ...
    def add_batch(self, predictions, references):
        ...
        Returns:
            None
        """
        preds_list, refs_list, preds_idx_list, refs_idx_list = batch_to_relational_lists(predictions, references)
        
        with Pool(self.n_processes) as p:
            samplewise_metrics = p.starmap(lambda x,y,x_idx,y_idx : relational_metric(x[:x_idx], y[:y_idx]), zip(preds_list, refs_list, preds_idx_list, refs_idx_list))
        
        common_rels, total_rels = reduce(lambda tup1, tup2: (tup1[0]+tup2[0], tup1[1]+tup2[1]), samplewise_metrics, (0,0))
    ...
    def compute(self):
        ...
        if self.total_relations==0:
            raise AssertionError("No sample with relations to compute metric over was added. Use metric.add_batch(preds, refs) to add batches first.")
        
        result = {'intersection': self.common_relations,
...
def batch_to_post_tags(references: jnp.ndarray, predictions: jnp.ndarray) -> Tuple[List[List[str]], List[List[str]]]:
    """
    ...
    seq_lens = jnp.sum(references!=config['pad_for']['post_tags'], axis=-1).tolist()
    
    def convert_ids_to_tags(lis, idx):
        return ['B-P' if config['post_tags']['B']==lis[i] else 'I-P' for i in range(0, idx)]

    with Pool(sum(seq_lens)//10000+1) as p:
        predictions_lis = p.starmap(convert_ids_to_tags, zip(predictions.tolist(), seq_lens))
        references_lis = p.starmap(convert_ids_to_tags, zip(references.tolist(), seq_lens))
