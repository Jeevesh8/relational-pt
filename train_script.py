...
@partial(max_comps=stable_config['max_comps'], embed_dim=stable_config['embed_dim'])
...
def predict_components(logits, lengths, label_tags):
    return crf_layer(n_classes=2).batch_viterbi_decode(hk.Linear(2)(logits), lengths, label_tags)[0]
...
def train_step(state, batch, key):
    ...
    def comp_prediction_loss(params, key):
        ...
        logits = state.apply_fn(batch.input_ids, attention_mask, params=params['embds_params'], dropout_rng=subkey, train=True)
        return state.comp_prediction_loss(params['comp_predictor'], key, logits, lengths, batch.post_tags)
    ...
    def relation_prediction_loss(params, key):
        key, subkey = jax.random.split(key)
        embds = state.apply_fn(batch.input_ids, attention_mask, batch.post_tags, params=params['embds_params'], dropout_rng=subkey, train=True)
        return state.relation_prediction_loss(params['relation_predictor'], key, embds, batch.post_tags==config['post_tags']['B'], batch.relations)
    ...
    losses = {'comp_pred_loss' : jax.lax.pmean(_comp_prediction_loss, axis='device_axis'), 
              'rel_pred_loss': jax.lax.pmean(_relation_prediction_loss, axis='deivce_axis')}
...
def get_preds(state, batch, key):
    ...
    logits = state.apply_fn(batch.input_ids, attention_mask, params=params['embds_params'], dropout_rng=key, train=False)
    
    comp_preds = state.comp_predictor(params['comp_predictor'], key, logits, lengths, batch.post_tags)

    embds = state.apply_fn(batch.input_ids, attention_mask, batch.post_tags, params=params['embds_params'], dropout_rng=key, train=False)
    
    rel_preds = state.relation_prediction_loss(params['relation_predictor'], key, embds, batch.post_tags==config['post_tags']['B'])
...
if __name__=='__main__':
    ...
    transformer_model = FlaxBigBirdModel.from_pretrained(stable_config['checkpoint'])
    ...
    sample_logits = jnp.zeros((config['batch_size'], stable_config['max_len'], config['embed_dim']), dtype=jnp.float32)
    sample_lengths = jnp.full((config['batch_size']), stable_config['max_len'], dtype=jnp.int32)
    sample_comp_labels = jax.random.randint(key, (config['batch_size'], stable_config['max_len']), 0, 2)
    sample_relations = jax.random.randint(key, (stable_config['max_comps'], 3), 0, stable_config['max_comps'])
    sample_relations = jnp.where(jnp.array([True, True, False]), sample_relations, 0)

    key, subkey = jax.random.split(key)
    params['comp_predictor'] = pure_cpl.init(subkey, sample_logits, sample_lengths, sample_comp_labels)

    key, subkey = jax.random.split(key)
    params['relation_predictor'] = pure_rpl.init(subkey, sample_comp_labels==0, sample_relations)

    params['embds_params'] = transformer_model.params

    opt = optax.chain(optax.adam(learning_rate=config['lr']))

    init_train_state = train_state.TrainState.create(apply_fn=transformer_model.__call__,
                                                     params=params,
                                                     tx=opt,
    ...
    losses = {'comp_pred_loss' : jnp.array([0.]),
              'rel_pred_loss': jnp.array([0.])}
    ...
    for epoch in range(config['n_epochs']):
        for batch in train_dataset:
            loop_state, step_losses, key = parallel_train_step(loop_state, batch, key)
            losses = jax.tree_multimap(lambda x, y : x+y, losses, step_losses)
            num_iters += 1
            
            if num_iters%log_loss_n_iters==0:
                print("component prediction loss for iterations", num_iters-log_loss_n_iters,"to", num_iters, ":", losses['comp_pred_loss']/log_loss_n_iters)
                print("relation prediction loss for iterations", num_iters-log_loss_n_iters,"to", num_iters, ":", losses['rel_pred_loss']/log_loss_n_iters)
                losses['comp_pred_loss'] = 0.
                losses['rel_pred_loss'] = 0.
            
            if num_iters%validation_n_iters==0:
                for batch in val_dataset:
                    key, subkey = jax.random.split(key)
                    eval_step(loop_state, batch, subkey)
                print("Component Prediction metrics for iterations", num_iters, "to", num_iters-validation_n_iters,":", comp_prediction_metric.compute())
                print("Relation Prediction metrics for iterations", num_iters, "to", num_iters-validation_n_iters,":", rel_prediction_metric.compute())
